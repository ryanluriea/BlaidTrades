  ...
  if (raw.linkedBot) {
    ...
    linkedBot = {
      ...
      stageMetrics: lb.stageMetrics ? {
        ...
        sharpeRatio: lb.stageMetrics.sharpeRatio ?? null,
        maxDrawdownPct: lb.stageMetrics.maxDrawdownPct ?? null,
      } : null,
      createdAt: lb.createdAt ?? lb.created_at ?? null,
    };
  }
  
  // Defensive: use null/safe defaults for display-only fields, NOT fabricated business values
  // Empty string for IDs/names prevents crashes while showing "Unknown" in UI
  return {
    id: raw.id ?? "",
    strategyName: raw.strategy_name ?? "",
    archetypeName: raw.archetype_name ?? null,
    ...
    regimeTrigger: raw.regime_trigger ?? null,
    source: raw.source as StrategyCandidate["source"],
    sourceLabBotId: raw.source_lab_bot_id ?? null,
    createdBotId: raw.created_bot_id ?? null,
    lineageChain: raw.lineage_chain || undefined,
    recycledFromId: raw.recycled_from_id || null,
    // Defensive: provide empty string defaults for rules to prevent undefined rendering
    rulesJson: {
    ...
    regimeAdjustment: raw.regime_adjustment ? {
      ...
      regimeMatch: raw.regime_adjustment.regime_match ?? "NEUTRAL",
      reason: raw.regime_adjustment.reason ?? "",
      currentRegime: raw.regime_adjustment.current_regime ?? "UNKNOWN",
    } : null,
