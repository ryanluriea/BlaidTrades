...
- **FK Migration Always Runs (Jan 12):** Fixed critical bug where seed-system-users.ts would exit early if DEFAULT_USER_ID already existed, skipping FK migration entirely. Now the script ALWAYS runs `migrateOrphanedFKReferences()` which scans all FK columns referencing users.id and migrates any data tied to wrong user IDs. Logs show exactly which tables/columns had rows migrated.
- **Tick Table Startup Migration (Jan 12):** Added `ensureTickTablesExist()` function in server/db.ts that creates tick ingestion tables (trade_ticks, quote_ticks, order_book_snapshots, tick_sequence_gaps, tick_ingestion_metrics) using CREATE TABLE IF NOT EXISTS on every server startup. Resolves issue where Drizzle db:push wasn't creating these tables in production. Migration is idempotent, handles partial failures gracefully, and logs which tables were created.
- **User ID Migration UNIQUE Constraint Fix (Jan 12):** Fixed seed-system-users.ts to handle email UNIQUE constraint during user ID migration. When migrating a user from wrong ID to DEFAULT_USER_ID: (1) temporarily changes old user's email to placeholder, (2) inserts new user with DEFAULT_USER_ID and original email (preserving ALL columns dynamically via information_schema), (3) migrates all FK references, (4) deletes old user. All within a single transaction. This resolves the duplicate key violation that was blocking production deployment.
- **INTEGRATION_FAILURE Enum (Jan 12):** Added INTEGRATION_FAILURE to alert_category enum in schema.ts and database to enable self-healing worker to categorize integration-related alerts properly.
- **Tick Ingestion Metrics Schema Fix (Jan 12):** Fixed column name mismatch between ensureTickTablesExist() CREATE TABLE and Drizzle schema. Now correctly uses: window_start, window_end, window_duration_ms, trade_tick_count, quote_tick_count, latency_us fields (not latency_ns), gaps_resolved, stale_tick_count, out_of_order_count. Added DROP TABLE IF EXISTS before CREATE to force schema refresh on production.
- **Production Password Reset (Jan 12):** Added resetPasswordIfRequested() function to seed-system-users.ts. When RESET_USER_PASSWORD env var is set, it resets the user password during deploy. Works in all three code paths: user exists with correct ID, user exists with wrong ID, or new user creation.
- **Explicit User ID Migration (Jan 12):** ROOT CAUSE FIX for empty bots page and missing QC badges. Added Step 3 to migrateOrphanedFKReferences() that explicitly migrates orphaned user_id columns in tables WITHOUT FK constraints: bots, strategy_candidates, qc_verifications, bot_generations, bot_instances, bot_live_metrics, backtest_sessions, activity_events, backtests, backtest_queue, trade_logs, bar_cache. Only migrates rows where user_id points to a non-existent user (NOT IN users table), preserving legitimate multi-user data.
- **Startup User Consolidation (Jan 12):** FINAL FIX - Added `ensureCanonicalUserConsolidation()` function in server/db.ts that runs on EVERY startup. Uses information_schema to dynamically find ALL 30 FK columns referencing users table, migrates all data to canonical user (blaidtrades@gmail.com), then deletes all other users. Function is idempotent - only runs migration if other users exist. Successfully migrated 358 rows (302 audit_reports, 56 evolution_tournaments) and deleted 2 orphan users. Confirmed 239 bots now owned by canonical user. This resolves the empty bots page in production by ensuring data ownership consolidation regardless of which database hosts which user_ids.
- **Consolidation Retry Fix (Jan 12):** Fixed issue where consolidation wasn't running on Render production because warmupDatabase() failed and consolidation was inside `if (dbReady)` block. Moved consolidation to independent IIFE after server.listen() with its own 5-retry exponential backoff (2s, 4s, 8s, 16s, 32s). Now runs regardless of warmupDatabase() result, ensuring production data consolidation even under degraded startup conditions.
